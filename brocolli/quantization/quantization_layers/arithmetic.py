...
@register_quant_op("sub")
class Sub(nn.Module, BaseOperator):
    def __init__(self):
    ...
    def from_float(cls, mod):
        ...
        qsub = cls()
        qsub.qbit = mod.activation_post_process.qbit
        qsub.act_scale1 = float(act_scale1)
        qsub.act_scale2 = float(act_scale2)
        qsub.output_scale = float(output_scale)
        qsub.output_min_value = activation_post_process.min_val
        qsub.output_max_value = activation_post_process.max_val
