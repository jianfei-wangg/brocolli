...
class _ConvNd(nn.Module):
    ...
    def get_qconv(
        ...
        qconv.act_scale = torch.nn.Parameter(act_scale, requires_grad=False).to(qweight.device)
        qconv.wt_scale = torch.nn.Parameter(wt_scale.reshape(1, -1, 1, 1), requires_grad=False).to(qweight.device)
        qconv.output_scale = torch.nn.Parameter(output_scale, requires_grad=False).to(qweight.device)
